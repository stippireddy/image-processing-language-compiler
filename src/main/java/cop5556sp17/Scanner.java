...
public class Scanner {
                        ...
                        "-"), TIMES("*"), DIV("/"), MOD("%"), NOT("!"), ASSIGN("<-"), OP_BLUR(
                            "blur"), OP_GRAY("gray"), OP_CONVOLVE("convolve"), KW_SCREENHEIGHT(
                                "screenheight"), KW_SCREENWIDTH("screenwidth"), OP_WIDTH(
                                    "width"), OP_HEIGHT("height"), KW_XLOC(
                                        "xloc"), KW_YLOC("yloc"), KW_HIDE("hide"), KW_SHOW(
                                            "show"), KW_MOVE("move"), OP_SLEEP(
                                                "sleep"), KW_SCALE("scale"), EOF("eof");
  ...
  public class Token {
    ...
    /**
     * Precondition: kind = Kind.INT_LIT, the text can be represented with a Java int. Note that the
     * validity of the input should have been checked when the Token was created. So the exception
      ...
      switch (state) {
            ...
            default:
              if (Character.isDigit(ch)) {
                ...
                throw new IllegalCharException(
                    "The character " + ch + " is not recognized by this language");
              }
              break;
          }
          break;
        ...
        case IN_EQUAL:
          if (ch == '=') {
            ...
            throw new IllegalCharException("The literal " + chars.substring(startPos, pos)
                + " is not recognized by this language");
          }
  ...
  /*
   * Return the next token in the token list and update the state so that the next call will return
   * the Token..
   */
  ...
  /*
   * Return the next token in the token list without updating the state. (So the following call to
   * next will return the same token.)
   */
  public Token peek() {
    if (tokenNum >= tokens.size())
      return null;
