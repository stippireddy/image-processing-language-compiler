...
public class TypeCheckVisitor implements ASTVisitor {
  ...
  public Object visitBinaryChain(BinaryChain binaryChain, Object arg) throws Exception {
    ...
    if (typeOfChain0 == TypeName.URL && opKind == Kind.ARROW && typeOfChain1 == TypeName.IMAGE) {
    ...
    } else if (typeOfChain0 == TypeName.FILE && opKind == Kind.ARROW
        && typeOfChain1 == TypeName.IMAGE) {
      binaryChain.setType(TypeName.IMAGE);
    } else if (typeOfChain0 == TypeName.FRAME && opKind == Kind.ARROW
        && (chain1 instanceof FrameOpChain && (chain1.getFirstToken().isKind(Kind.KW_XLOC)
            || chain1.getFirstToken().isKind(Kind.KW_YLOC)))) {
      binaryChain.setType(TypeName.INTEGER);
    } else if (typeOfChain0 == TypeName.FRAME && opKind == Kind.ARROW
        && (chain1 instanceof FrameOpChain && (chain1.getFirstToken().isKind(Kind.KW_SHOW)
            || chain1.getFirstToken().isKind(Kind.KW_HIDE)
      ...
      binaryChain.setType(TypeName.FRAME);
        ...
        && (chain1 instanceof ImageOpChain && (chain1.getFirstToken().isKind(Kind.OP_WIDTH)
            || chain1.getFirstToken().isKind(Kind.OP_HEIGHT)))) {
      binaryChain.setType(TypeName.INTEGER);
    } else if (typeOfChain0 == TypeName.IMAGE && opKind == Kind.ARROW
        && typeOfChain1 == TypeName.FRAME) {
      binaryChain.setType(TypeName.FRAME);
    } else if (typeOfChain0 == TypeName.IMAGE && opKind == Kind.ARROW
        && typeOfChain1 == TypeName.FILE) {
      binaryChain.setType(TypeName.NONE);
    } else if (typeOfChain0 == TypeName.IMAGE && (opKind == Kind.ARROW || opKind == Kind.BARARROW)
        && (chain1 instanceof FilterOpChain && (chain1.getFirstToken().isKind(Kind.OP_GRAY)
            || chain1.getFirstToken().isKind(Kind.OP_BLUR)
      ...
      throw new TypeCheckException(
          "Incompatible chain types encountered for the given binary chain.");
    }
    ...
    switch (opKind) {
      ...
      case MINUS:
        if (t0 == TypeName.INTEGER && t1 == TypeName.INTEGER) {
          ...
          throw new TypeCheckException(
              "Incompatible expression types encountered for the given operation.");
        }
        break;
      case TIMES:
        if (t0 == TypeName.INTEGER && t1 == TypeName.INTEGER) {
  ...
  public Object visitFrameOpChain(FrameOpChain frameOpChain, Object arg) throws Exception {
    ...
    } else if (opChainKind == Kind.KW_XLOC || opChainKind == Kind.KW_YLOC) {
      if (tupleSize != 0) {
        throw new TypeCheckException("The length of the expression list is more than expected.");
      }
  ...
  public Object visitIdentChain(IdentChain identChain, Object arg) throws Exception {
    ...
    if (dec == null) {
      throw new TypeCheckException(
          "The identifier attempted to be found does not exist in the current scope.");
    } else {
  ...
  public Object visitDec(Dec declaration, Object arg) throws Exception {
    ...
    if (isDeclaredProperly == false) {
      throw new TypeCheckException(
          "Multiple declarations of a variable in a single scope is not allowed.");
    }
    return null;
  }
  ...
  public Object visitParamDec(ParamDec paramDec, Object arg) throws Exception {
    ...
    if (isParamDeclaredProperly == false) {
      throw new TypeCheckException(
          "Multiple parameter declarations in a single scope is not allowed.");
    }
    return null;
  }
