...
public class CodeGenVisitor implements ASTVisitor, Opcodes {
	...
	public Object visitProgram(Program program, Object arg) throws Exception {
		...
		for (Dec declaration : listOfDeclarations) {
			mv.visitLocalVariable(declaration.getIdent().getText(), classDesc, null, declaration.getScopeStart(),
					declaration.getScopeEnd(), declaration.getSlot());
		}
		mv.visitMaxs(1, 1);
	...
	public Object visitBinaryChain(BinaryChain binaryChain, Object arg) throws Exception {
		...
		if (binaryChain.getArrow().kind == Kind.BARARROW && binaryChain.getE1() instanceof FilterOpChain) {
	...
	private void paramDeclarationVisitCommon(MethodVisitor mv, String paramName, String methodOwner, String methodName,
			String methodDesc, TypeName type) {
		mv.visitVarInsn(ALOAD, 0);
		mv.visitVarInsn(ALOAD, 1);
		mv.visitLdcInsn(fieldCount);
		mv.visitInsn(AALOAD);
	...
	public Object visitBinaryExpression(BinaryExpression binaryExpression, Object arg) throws Exception {
		...
		if (t == TypeName.BOOLEAN) {
			...
			if (t0 == TypeName.IMAGE && t1 == TypeName.IMAGE) {
				switch (opKind) {
				case PLUS:
					mv.visitMethodInsn(INVOKESTATIC, PLPRuntimeImageOps.JVMName, "add", PLPRuntimeImageOps.addSig,
							false);
					break;
				case MINUS:
					mv.visitMethodInsn(INVOKESTATIC, PLPRuntimeImageOps.JVMName, "sub", PLPRuntimeImageOps.subSig,
							false);
					break;
				default:
					throw new UnsupportedOperationException(
				...
				switch (opKind) {
				case TIMES:
					mv.visitMethodInsn(INVOKESTATIC, PLPRuntimeImageOps.JVMName, "mul", PLPRuntimeImageOps.mulSig,
							false);
					break;
				case DIV:
					mv.visitMethodInsn(INVOKESTATIC, PLPRuntimeImageOps.JVMName, "div", PLPRuntimeImageOps.divSig,
							false);
					break;
				case MOD:
					mv.visitMethodInsn(INVOKESTATIC, PLPRuntimeImageOps.JVMName, "mod", PLPRuntimeImageOps.modSig,
							false);
					break;
				default:
					throw new UnsupportedOperationException(
	...
	public Object visitIdentExpression(IdentExpression identExpression, Object arg) throws Exception {
		if (identExpression.getDec() instanceof ParamDec) {
			...
			if (identExpression.getType() == TypeName.IMAGE || identExpression.getType() == TypeName.FRAME) {
